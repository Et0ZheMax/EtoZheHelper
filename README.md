...
Chat responses may include parameter-free suggested action cards such as `resolved_status`, `disk_usage`, `inode_usage`, `failed_units`, `docker_ps_all`, `docker_system_df`, `cups_status`, and `lpstat_all`; the UI renders command previews as plain text. Execution is available only after Prepare → Approve → Check readiness for approved read-only ActionRuns.

## Prepared Action Runs

Stage 11 added Prepared Action Runs: an auditable preview layer between action proposals and the Stage 14 approved read-only SSH executor. A prepared run is created from `session_id`, `host_id`, an allowlisted `action`, and validated `params`. The server validates the optional chat session, validates that the optional host exists and is enabled, passes the action and params through the existing Action Policy Engine, then saves only the resulting command preview.

`Prepare run` in the UI means **validate and save a preview**. It does not run the command. Prepared runs always have `status: "prepared"` and `execution_enabled: false` until an operator explicitly approves and readiness passes. The command preview is generated only by `propose_action()` from allowlisted catalog templates; arbitrary client-provided command strings are rejected by the request schema.
...
Every prepared run writes an `action_run_prepared` audit event with run id, session id, host id, action, category, risk, and `execution_enabled:false`. Preparation itself still does not connect over SSH, does not invoke subprocess/shell, does not perform reachability checks, does not read private key files, and does not store secrets. Stage 14 execution is a separate approved read-only SSH-agent-only path.
...
Hosts require an assigned SSH profile before readiness can pass. Stage 16 adds UI management for agent-based SSH profiles and host assignment. Agent profiles store no secrets; the operator must ensure the key is loaded in the OS ssh-agent and the host key is trusted in `known_hosts`.

Supported executable profile for Stage 14/15:

- `auth_type`: `agent`
- `sudo_mode`: `none`

Other auth/sudo modes remain metadata-only or blocked for execution. The web UI is not an SSH terminal: it does not provide arbitrary command input, shell access, sudo password handling, key-file execution, or remediation/write actions.

Manual UI smoke:

1. Add host.
2. Add agent SSH profile.
3. Assign profile to host.
4. Select host.
5. Ask for a read-only check.
6. Prepare run.
7. Approve.
8. Check readiness.
9. Run read-only SSH check.
10. Review stdout/stderr and analysis.

Before real SSH, run once from the same OS user if needed:

```powershell
ssh support@mlepeshev-vb
```

Accept the host fingerprint, then exit.

## Packaging clean archive

Before creating a release zip, remove local Python/test caches:
...
Then create archive from project files only:

```powershell
Compress-Archive `
  -Path app,knowledge_base,scripts,tests,README.md,pyproject.toml,.env.example,.gitignore `
  -DestinationPath EtoZheHelper-final.zip `
  -Force
```
...
The UI has `Hosts` and `SSH Profiles` panels. Operators can add host metadata, create agent/none SSH profile metadata records, assign a profile to a host, and select the current host context for the investigation. Selected host context is persisted on the chat session as metadata only. Preparing a run still stores a preview first; execution remains limited to approved read-only ActionRuns that pass readiness.
...
Stage 14/15 supports only approved read-only SSH checks through SSH agent auth and `sudo_mode=none`; there is still no arbitrary SSH terminal, shell executor, local command execution, reachability scanner, Ansible/Docker/Terraform execution, external LLM/API, or outbound user-data upload. Host Inventory и SSH Profiles хранят только metadata/non-secret references. Selected host context is persisted as metadata and is used only by the approved ActionRun readiness/execution flow. Любые команды и action cards в ответах — это previews/текстовые подсказки до явного Prepare → Approve → Check readiness → Run read-only SSH check.
...
- Нет произвольного SSH terminal, shell execution и запуска пользовательских команд; поддерживаются только approved read-only SSH checks через SSH agent.
