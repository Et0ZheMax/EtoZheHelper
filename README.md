...
Stage 14.5 improves execution observability: blocked executions return an execution id, latest executions can be reviewed from the UI, stdout/stderr can be copied, and common SSH failures are classified with safe operator-friendly messages. Analyzer integration is intentionally skipped for now and can be added in a future deterministic local-only stage.
