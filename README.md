...
Stage 10 намеренно не содержит SSH, shell executor, remote execution, local command execution, Ansible/Docker/Terraform execution, внешних LLM/API и отправки пользовательских данных наружу. Любые команды и action cards в ответах — это только previews/текстовые подсказки для оператора, а не действия приложения.
